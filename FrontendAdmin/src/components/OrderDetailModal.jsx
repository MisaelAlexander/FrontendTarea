 ...
 * Muestra información completa: productos, cantidades, montos, totales y datos del comprador.
 */
  ...
  return (
    <div className="bg-white min-h-[500px]">
      ...
      <div className="bg-gradient-to-r from-[#4a69bd] to-[#3c40c6] p-8 pb-10 relative">
        ...
        <h2 className="text-white text-3xl font-bold">Información del Pedido #{order.numeroPedido || ''}</h2>
      </div>
      ...
      <div className="p-8 -mt-6 bg-white rounded-t-[3rem] relative z-10 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex items-center justify-between border-2 border-black rounded-xl px-4 py-3">
            ...
            <span className="text-gray-600 text-sm">{order.tipoPedido || 'Online'}</span>
          </div>
          <div className="flex items-center justify-between border-2 border-black rounded-xl px-4 py-3">
          ...
          <div className="flex items-center justify-between border-2 border-black rounded-xl px-4 py-3">
            ...
            <span className="text-gray-600 text-sm capitalize">{order.tipoPago}</span>
          </div>
        </div>

        {/* Sección Central: Tabla de Productos y Datos del Comprador */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 border-2 border-black rounded-2xl p-6 overflow-hidden">
              ...
              {order.items && order.items.length > 0 ? (
                order.items.map((item, idx) => (
                  <div key={idx} className="grid grid-cols-4 text-sm text-gray-700 items-center">
                    ...
                    <span className="text-right font-mono">$ {Number(item.monto).toFixed(2)}</span>
                  </div>
                ))
              ) : (
            ...
            <div className="mt-8 pt-6 border-t-2 border-gray-100 space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono">${Number(order.descuento || 0).toFixed(2)}</span>
              </div>
          ...
          {/* Lateral derecho: Comprador, Estado, Tipo de Retiro */}
