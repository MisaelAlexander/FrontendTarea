...
const EmployeeFormModal = ({ user, onClose, onSave }) => {
  ...
  const { register, handleSubmit, control, setValue, watch, formState: { errors } } = useForm({
    defaultValues: {
      ...
      // eliminado fecha
      rol: user?.tipo === 'vendedor' ? 'Vendedor' : user?.tipo === 'repartidor' ? 'Repartidor' : 'Vendedor',
      salario: user?.salario || '',
      sucursal: user?.sucursal || '',
      password: '',
      fotoPerfilPreview: user?.fotoPerfil || null,
      duiFrontalPreview: user?.DUI?.[0]?.imagenDUI || null,
      duiTraseraPreview: user?.DUI?.[1]?.imagenDUI || null,
      fotoPerfilFile: null,
      duiFrontalFile: null,
      duiTraseraFile: null,
    },
  ...
  const onSubmit = (data) => {
    ...
    const id = user?._id;
    const rol = data.rol === 'Vendedor' ? 'vendedor' : 'repartidor';
    onSave(formData, esEdicion, id, rol);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {esEdicion ? (
                <>
                  <div className="w-full px-4 py-3 border-2 border-gray-200 rounded-2xl bg-gray-50 text-gray-700 font-medium">
                    {user.tipo === 'vendedor' ? 'Vendedor' : 'Repartidor'}
                  </div>
                ...
                <Controller
                  name="rol"
                  control={control}
                  rules={{ required: 'Debe seleccionar un cargo' }}
                  render={({ field }) => (
                    <div className="flex gap-2 flex-wrap">
                      {['Vendedor', 'Repartidor'].map(role => (
                        <button
                          type="button"
                          key={role}
                          onClick={() => field.onChange(role)}
                          className={`px-4 py-2 rounded-2xl border-2 font-medium ${
                            field.value === role ? 'border-blue-500 bg-blue-50 text-blue-600' : 'border-gray-200 text-gray-500'
                          }`}
                        >
                          {role}
                        </button>
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex gap-4">
              ...
              <div
                ...
                className={`flex-1 border-2 border-dashed rounded-2xl aspect-[1.6/1] flex items-center justify-center overflow-hidden ${!esEdicion ? 'cursor-pointer' : ''}`}
              >
                {!esEdicion && (
                ...
                {watch('duiFrontalPreview') ? (
                  <img src={watch('duiFrontalPreview')} className="w-full h-full object-cover" />
                ) : (
              ...
              >
                ...
                {watch('duiTraseraPreview') ? (
                  <img src={watch('duiTraseraPreview')} className="w-full h-full object-cover" />
                ) : (
