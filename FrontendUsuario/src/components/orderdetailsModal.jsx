...
const InfoBox = ({ label, value }) => (
  <div className="border border-gray-300 rounded-lg px-4 py-3 flex justify-between items-center bg-white flex-1">
    ...
    <span className="text-[13px] text-gray-600 truncate">{value || 'N/A'}</span>
  </div>
);
...
const OrderDetailsModal = ({ order, isOpen, onClose }) => {
  ...
  const productos = order.details?.productos || [];
  const descuento = Number(order.details?.descuento) || 0;
            ...
            <div className="md:col-span-2 border border-gray-300 rounded-xl p-5 flex flex-col bg-white">
              ...
              <div className="flex flex-col gap-3">
                {productos.length > 0 ? (
                  productos.map((prod, idx) => {
                    const nombre = prod.IDProducto?.nombre || prod.nombre || 'Producto';
                    const cantidad = prod.amount || prod.cantidad || 1;
                    const precio = Number(prod.IDProducto?.precio || prod.precio || prod.monto || 0);
                    const monto = precio * cantidad;

                    return (
                      <div key={idx} className="flex justify-between items-center">
                        <span className="text-[13px] text-gray-700 flex-1 pr-4 truncate">{nombre}</span>
                        <span className="text-[13px] text-gray-700 w-16 text-center">{cantidad}</span>
                        <span className="font-bold text-[13px] text-gray-800 w-24 text-right">$ {monto.toFixed(2)}</span>
                      </div>
                    );
                  })
                ) : (
                  <p className="text-[13px] text-gray-500 text-center py-4">Sin productos</p>
                )}
              </div>
              ...
              <div className="mt-10 flex flex-col gap-3 self-end w-full md:w-[60%]">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[13px] text-gray-700">$ {descuento.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                  <span className="font-extrabold text-[13px] text-gray-800">Total</span>
                  <span className="font-bold text-[13px] text-gray-800">$ {total.toFixed(2)}</span>
                </div>
              </div>
            </div>
